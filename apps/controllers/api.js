...
var db_travel_model = require("../models/db_travel_models");
...
//database: vflco_traveldocs
router.get("/traveldocs/get-data", (req, res) => {
    var secur_key = req.query.secur_key;
    if (secur_key === api_secur.db_travel_secur) {
        var db = req.query.db;
        if (db) {
            if (req.query.fields) {
                var fields = req.query.fields;
            ...
            db_travel_model.getData(db, fields, where, orderBy, limit)
                .then(data => {
                    res.json({ "mess": "ok", "data": data });
                })
                .catch(err => res.json({ "mess": "fail", "err": err }));
...
router.post("/traveldocs/add-data", jsonParser, (req, res) => {
    if (req.body) {
        var secur_key = req.body.secur_key;
        if (secur_key === api_secur.db_travel_secur) {
            var data_table = req.body.data_table;
            ...
            if (options.setNum) {
                ...
                db_travel_model.getData(data_table, 'id', where, '', '')
                    .then(data => {
                        fields.num = data.length + 1;
                        // res.json({fields: fields});
                        db_visas_model.addData(data_table, fields)
                ...
                db_travel_model.addData(data_table, fields)
                    .then(result => res.json({
                        "mess": "ok",
                        "result": result,
                        "code": fields.code,
...
router.post("/traveldocs/edit-data", jsonParser, (req, res) => {
    if (req.body) {
        let secur_key = req.body.secur_key;
        if (secur_key === api_secur.db_travel_secur) {
            let data_table = req.body.data_table;
            ...
            let where = req.body.where;
            let params = req.body.params;
            db_travel_model.editData(data_table, set, where, params)
                .then(result => res.json({ "mess": "ok", "result": result }))
                .catch(err => res.json({
                    "mess": "fail",
                    "err": err
...
router.post("/traveldocs/delete-data", jsonParser, (req, res) => {
    if (req.body) {
        let secur_key = req.body.secur_key;
        if (secur_key === api_secur.db_travel_secur) {
            let data_table = req.body.data_table;
            let where = req.body.where;
            db_travel_model.deleteData(data_table, where)
                .then(result => res.json({ "mess": "ok" }))
                .catch(err => res.json({
                    "mess": 'fail',
                    "err": err
